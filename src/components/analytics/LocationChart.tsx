    ...
    <div className="chart-container">
      <div className="mb-6 flex flex-col gap-1">
        ...
        <p className="text-sm text-muted-foreground">
          {total} usuários em {locations.length} regiões
        </p>
      </div>
      ...
      <div className="space-y-3">
        {locations.map((location, index) => {
          ...
          const displayName = location.city || location.state || "Não informado";
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">
                    ({Math.round((location.count / total) * 100)}%)
                  </span>
                </div>
              </div>
      ...
      <div className="mt-6 p-4 rounded-lg bg-muted/50 border border-border">
        <p className="text-sm text-muted-foreground">
          <span className="font-semibold text-foreground">Insight:</span> A maior concentração 
          de usuários está em <span className="font-semibold text-foreground">
            {locations[0]?.city || locations[0]?.state || "N/A"}
          </span>.
        </p>
      </div>
    </div>
