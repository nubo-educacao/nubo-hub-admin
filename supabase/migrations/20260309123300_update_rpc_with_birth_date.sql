-- Update update_own_profile to support birth_date
CREATE OR REPLACE FUNCTION public.update_own_profile(
    ...
    p_birth_date DATE DEFAULT NULL -- NEW PARAMETER
) 
RETURNS JSONB 
LANGUAGE plpgsql 
SECURITY DEFINER 
...
DECLARE
  ...
  v_new_birth_date DATE; -- NEW VARIABLE
...
BEGIN
  ...
  v_new_birth_date := COALESCE(p_birth_date, v_current_profile.birth_date); -- NEW COALESCE

  v_is_complete := (
    v_new_full_name IS NOT NULL AND length(v_new_full_name) > 0
    AND (v_new_age IS NOT NULL OR v_new_birth_date IS NOT NULL) -- Accept either for completion
  ...
  UPDATE public.user_profiles SET
    ...
    birth_date = v_new_birth_date, -- NEW COLUMN
